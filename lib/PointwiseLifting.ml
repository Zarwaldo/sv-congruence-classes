(* $Id: c60d29278bc2c442b6158008412c724d570d21a9 $ *)
...
struct
  ...
  (*
     ...
     val eval : t -> Command.Expression.t -> N.t

     This function inductively evaluates a given expression using the abstract
     numerical operations of the numerical non-relational abstract domain N.
   *)
  let rec eval a =
    function
    ...
    | Command.Expression.Op (e, o, e') ->
       (* Abstract numerical function corresponding to the op. *)
       let fun_op =
         match o with
  ...
  (*
     ...
     val post_assign : Variable.t -> Command.Expression.t -> t -> t
  ...
  (*
     ...
     val linearize : Command.Expression.t -> t -> Variable.t -> N.t * N.t
  ...
  (*
     ...
     val feasible :
       Command.Expression.t -> (N.t -> N.t -> N.t -> N.t) -> N.t -> t -> bool
     ...
     N.Op.inequality.  The abstract context a is assumed to be non-empty.
  ...
  (*
     ...
     val refine :
       Command.Expression.t -> (N.t -> N.t -> N.t -> N.t) -> N.t -> t ->
       Variable.t -> t
  ...
  let refine e solver y a v =
    (* Optimization: If a(v) = ⊥ then v cannot be refined any further in a. *)
    if N.leq (get a v) N.bot then a
    else
      let (r, s) = linearize e a v
      in
  ...
  (*
     Helper function for post_guard.

     val post_guard_solve :
       Command.Expression.t -> (N.t -> N.t -> N.t -> N.t) ->
       Command.Expression.t -> t -> t

     post_guard_solve e solver e' a returns the abstract post-image of the guard
     e ⋈ e' from the abstract context a, where ⋈ is = or ≤ depending on solver.
   *)
  let post_guard_solve e solver e' a =
    if empty a then bot
    (* Optimization: If a(v) = ⊥ for all v then a cannot be refined further. *)
    else if leq a bot then a
    else
      let expr = expr_sub e e'
      and zero = N.abs 0
      in
      ...
      if feasible expr solver zero a then
        ...
        List.fold_left
          ...
          a
          (Command.Expression.variables expr)
      else
        bot
  ...
  (*
     ...
     val post_guard : Command.Predicate.t -> t -> t
   *)
  let rec post_guard (e, o, e') a =
    match o with
    | Command.Predicate.Eq ->
       post_guard_solve e N.Op.equality e' a
    | Command.Predicate.Lst ->
       post_guard_solve (expr_add e (expr_cst 1)) N.Op.inequality e' a
    | Command.Predicate.Gst ->
       post_guard (e', Command.Predicate.Lst, e) a
    | Command.Predicate.Leq ->
       post_guard_solve e N.Op.inequality e' a
    | Command.Predicate.Geq ->
       post_guard (e', Command.Predicate.Leq, e) a
  ...
  (*
     ...
     val pre_assign : Variable.t -> Command.Expression.t -> t -> t
